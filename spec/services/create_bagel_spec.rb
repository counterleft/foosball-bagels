...
describe CreateBagel do
  ...
  context "#save" do
    it "should save only if players involved are distinct" do
      actual = CreateBagel.save('2009/01/02', player1.name, player2.name, player3.name, player1.name)
      expect(actual.persisted?).to be_false
    end

    it "should create a new bagel given valid attributes" do
      expect {
        CreateBagel.save('2009/01/02', player1.name, player2.name, player3.name, player4.name)
      }.to change { Bagel.count }.by(1)
    end

    it "should have baked_on date on save" do
      actual = CreateBagel.save(nil, player1.name, player2.name, player3.name, player4.name)
      expect(actual.persisted?).to be_false
    end

    it "should have defensive opponent on save" do
      actual = CreateBagel.save('2009/01/02', player1.name, player2.name, player3.name, nil)
      expect(actual.persisted?).to be_false
    end

    it "should have offensive opponent on save" do
      actual = CreateBagel.save('2009/01/02', player1.name, player2.name, nil, player4.name)
      expect(actual.persisted?).to be_false
    end

    it "should have teammate on save" do
      actual = CreateBagel.save('2009/01/02', player1.name, nil, player3.name, player4.name)
      expect(actual.persisted?).to be_false
    end

    it "should have owner on save" do
      actual = CreateBagel.save('2009/01/02', nil, player2.name, player3.name, player4.name)
      expect(actual.persisted?).to be_false
    end
